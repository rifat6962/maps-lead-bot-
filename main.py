...
# ════════════════════════════════════════════════════
#   ADVANCED KEYWORD ENGINE  (UNCHANGED)
# ════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════
#   DEEP EMAIL EXTRACTOR  (UNCHANGED)
# ════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════
#   AI KEYWORD GENERATOR  (UNCHANGED)
# ════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════
#   AI EMAIL PERSONALIZER  (UNCHANGED)
# ════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════
#   MASTER JOB RUNNER  (FIXED WITH LOGS & STOP FLAG)
# ════════════════════════════════════════════════════
def run_job_thread(job_id: str, data: dict):
    try:
        ...
        # ── State Persistence & UI Stop Features ──
        jobs[job_id] = {
            'status':       'scraping',
            'count':        0,
            'leads':        [],
        ...
        def _process_lead_batch(raw_leads: list, current_kw: str) -> bool:
            ...
            for lead in raw_leads:
                ...
                logger.info(f"[LEAD] ✅ #{jobs[job_id]['count']}/{max_leads} '{lead['Name']}' | rating={lead['Rating']} | {extracted_email}")
                jobs[job_id]['status_text'] = f"✅ {jobs[job_id]['count']}/{max_leads} leads found! Latest: {lead['Name']} ({extracted_email})"
        ...
        while len(jobs[job_id]['leads']) < max_leads:
            ...
            logger.info(f"[JOB] ── Scraping keyword {jobs[job_id]['stats']['keywords_used']}: '{current_kw}' | leads so far: {len(jobs[job_id]['leads'])}/{max_leads}")
            ...
            logger.info(f"[JOB] Qualified leads count: {len(jobs[job_id]['leads'])}/{max_leads} — continuing to next keyword")
  ...
  const poll = async () => {
    try {
      ...
      if(d2.status==='scraping') {
        ...
        // FIX: Always call showPV(d2.leads) to dynamically update the table
        if(d2.leads?.length){ 
            updStats(d2.leads); 
            showPV(d2.leads); 
            tableShown=true; 
