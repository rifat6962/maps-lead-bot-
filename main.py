...
def run_job_thread(job_id: str, data: dict):
    try:
        ...
        def _process_lead_batch(raw_leads: list, current_kw: str) -> bool:
            ...
            for lead in raw_leads:
                ...
                logger.info(
                    ...
                    f"rating={lead['Rating']} | website={lead['Website']}"
                )
                ...
                if is_valid_business_website(existing_url):
                    ...
                    logger.info(f"[WEBSITE] Already valid for '{lead['Name']}': {website}")
                else:
        ...
        # MAIN LOOP — scrape first, keywords later
        while len(jobs[job_id]['leads']) < max_leads:
            if not pending_keywords:
                if not keywords_generated:
                    jobs[job_id]['status_text'] = f"Generating keyword expansion for '{base_keyword}'..."
                    logger.info(f"[JOB] STEP 5: Keyword expansion")
                    new_kws = kw_engine.generate_full_pool(base_keyword, location, used_keywords)
                    keywords_generated = True
                else:
                    jobs[job_id]['status_text'] = f"Re-generating keywords for '{base_keyword}'..."
            ...
            jobs[job_id]['status_text'] = f"Scraping: '{current_kw}' in '{location}'..."
            logger.info(
            ...
            if not raw_leads:
                logger.info(f"[JOB] No results for '{current_kw}'")
            ...
            if target_reached:
                logger.info(f"[JOB] STOP: target reached")
                break
